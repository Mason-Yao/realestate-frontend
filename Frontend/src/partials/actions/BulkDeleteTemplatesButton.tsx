...
import { bulkDeleteTemplates, listTemplates } from "../../apis/templates"
import { setPagedTemplates, setNextPageKey } from "../../slices/emailSlice"
import { setBanner } from "../../slices/configSlice"
import { logger } from "../../../../Shared/Utils"

function BulkDeleteTemplatesButton({ selectedItems, clearSelectedItems, clearIsCheck }) {
  const dispatch = useAppDispatch()
  const [dangerModalOpen, setDangerModalOpen] = useState(false)
  const emailState = useAppSelector((state) => state.email)
  const page = emailState.currentPage
  const currentPageKey = emailState.currentPageKey
  ...
  const confirmRemove = (e) => {
    ...
    bulkDeleteTemplates(selectedItems)
      .then((res) => {
        ...
        dispatch(
          setBanner({
            status: "success",
            content: `Templates deleted successfully.`,
          })
        )
      })
      .catch((err) => {
        dispatch(
          setBanner({
            status: "error",
            content: `Something wrong happened. Only part of templates deleted successfully.`,
          })
        )
      })
    e.stopPropagation()
      ...
      <ModalBlank id="danger-modal" modalOpen={dangerModalOpen} setModalOpen={setDangerModalOpen}>
        <div className="p-5 flex space-x-4">
          ...
          <div className="w-full">
            {/* Modal header */}
            <div className="mb-2">
              <div className="text-lg font-semibold text-slate-800">Delete {selectedItems.length} selected templates?</div>
            </div>
            {/* Modal content */}
            <div className="text-sm mb-10">
              <div className="space-y-2">
                <p>Are you sure to delete all selected templates? </p>
              </div>
            </div>
            {/* Modal footer */}
...
export default BulkDeleteTemplatesButton
