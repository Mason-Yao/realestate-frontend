...
import { listProperties } from "../../../apis/property"
import { useDebounce } from "../../../utils/customHooks"

function PropertySearch({ onChange }) {
  const [options, setOptions] = useState([])
  const [input, setInput] = useState("")
  const debouncedSearch = useDebounce(input, 100)
  ...
  //TODO: use search property in future
  useEffect(() => {
  ...
  const select = (option) => {
    ...
    setInput(option.address.street)
    setShowOptions(false)
  }
            ...
            options.map((option, i, arr) => {
              ...
              return (
                <li className={className} key={i} onClick={() => select(option)}>
                  {option.address.street + " " + option.address.suburb + " " + option.address.state}
                </li>
              )
            })
          ) : (
