  ...
  return (
    <div className="relative inline-flex">
      ...
      <Transition
        ...
        appear={undefined}
      >
        <div ref={dropdown} className="font-medium text-sm text-slate-600" onFocus={() => setDropdownOpen(true)} onBlur={() => setDropdownOpen(false)}>
          {options.map((option) => {
            return (
              <button
                key={option.name}
                tabIndex={0}
                className={`flex items-center w-full hover:bg-slate-50 py-1 px-3 cursor-pointer ${option.name === selected.name && "text-indigo-500"}`}
                onClick={() => {
                  setSelected(option)
                  setDropdownOpen(false)
                }}
